...
(svm_s_val_miss <- mean(val$Class!=predict(svm_sigmoid, newdata=val)))

#Test Set Prediction
prediction1 <- predict(svc, newdata = test)
prediction2 <- predict(svm_poly, newdata = test)
prediction3 <- predict(svm_radial, newdata = test)
prediction4 <- predict(svm_sigmoid, newdata = test)
mean(prediction1 == test$Class)
mean(prediction2 == test$Class)
mean(prediction3 == test$Class)
mean(prediction4 == test$Class)
export_svm <- data.frame(SVC = prediction1, SVMPoly =prediction2, SVMRadial = prediction3, SVMSigmoid = prediction4)
